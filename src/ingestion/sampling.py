...
def stratified_sample_from_csv(number_of_desired_rows):
    """
    ...
    # Goes through the csv in chunks to avoid memory overflow
    for chunk in pd.read_csv('data/raw/fake_news_corpus.csv', chunksize=10000, engine='python',
                             on_bad_lines='skip', index_col=0):
        # Iterates through each row in the chunk
        for _, row in chunk.iterrows():
            # Gets the classification label for the current row
            class_label = row['type']
            # Skips any unlabelled entries or rogue labels
            if class_label not in reservoirs:
                continue
            # Increments number of entries for the current rows class
            number_of_entries_per_class[class_label] += 1

            # Adds row to reservoir of the rows class if it's not full
            if len(reservoirs[class_label]) < class_max_reservoir[class_label]:
                reservoirs[class_label].append(row.to_frame().T)
            else:
                # Picks a random row in the dataset between 0 and the entries seen of the current rows class
                random_index_inside_reservoir = random.randint(0, number_of_entries_per_class[class_label] - 1)
                # If the index of that row is within the reservoir size then replace row at that index with current row
                if random_index_inside_reservoir < number_of_rows_per_class:
                    reservoirs[class_label][random_index_inside_reservoir] = row.to_frame().T

    # Combine all samples into a singular csv by concatenating all the reservoirs into a panda then converting it
...
def random_sample_from_csv(number_of_desired_rows):
    ...
    """
    # Increases the maximum CSV field size to avoid overflow errors
    csv.field_size_limit(min(sys.maxsize, 2_147_483_647))
    # Path to the fake news dataset
    fake_news_csv_filename = "data/raw/fake_news_corpus.csv"
    ...
    # Open the fake news dataset and takes a random sample rows
    with open(fake_news_csv_filename, encoding="utf-8", errors="replace", newline="") as news_csv_file:
        ...
        csv_rows = []  # Reservoir to hold the rows that make up the sample
        ...
        for index, row in enumerate(news_csv_reader):
            if index < number_of_desired_rows:
                # Adds row to reservoir if it's not full
                csv_rows.append(row)
            else:
                # Selects a row in the dataset between the first and the current index
                random_row_selection = random.randint(0, index)
                # If the index is within the reservoir size then replace row at that index with current row
